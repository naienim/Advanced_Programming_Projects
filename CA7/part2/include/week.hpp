...
public:
    ...
    float find_base_score_of_player(string new_name);
    ...
    void calculate_scores_for_matches();
};
...
public:
    ...
    bool sell_player(string name,int &budget);
    bool buy_player(shared_ptr<Player> new_player, int budget);
    shared_ptr<Player> find_captain(string name);
    int find_price_by_name(string name);
    bool is_complete();
    vector<string> find_players_name();
    void print_players(shared_ptr<Player> captain);
    int find_total_cost();
    ...
    float find_scores(vector<shared_ptr<Week>> weeks , int week_index , vector<shared_ptr<Player>> list_players , shared_ptr<Player> captain);
};
