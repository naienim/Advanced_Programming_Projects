...
void Team::add_player(shared_ptr<Player> _player)
{
    players.push_back(_player->make_copy());
}
...
void Team::print_players(vector<shared_ptr<Player>> list, bool rank, string player_role)
{
    ...
    for (int i = 0; i < new_list.size(); i++)
    {
        ...
        cout << " | score: " << setprecision(1) << new_list[i]->get_average_score() << setprecision(0);
        cout << " | cost: " << new_list[i]->get_price();
        new_list[i]->print_player_data();
    }
}
