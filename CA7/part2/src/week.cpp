...
float Week::find_base_score_of_player(string new_name)
{
    shared_ptr<Player> temp;
    for (int i = 0; i < matches.size(); i++)
    {
...
void Week::calculate_scores_for_matches()
{
    for (auto &match : matches)
        match->calculate_both_teams_scores();
}
...
bool MyTeam::sell_player(string name , int &budget)
{
    for (int i = 0; i < players.size(); i++)
        if (players[i]->get_name() == name)
        {
            budget += players[i]->get_price();
...
bool MyTeam::buy_player(shared_ptr<Player> new_player,int budget)
{
    for (int i = 0; i < players.size(); i++)
        if (players[i]->get_name() == new_player->get_name())
            return false;
...
int MyTeam::find_price_by_name(string name)
{
    for (int i = 0; i < players.size(); i++)
        if (players[i]->get_name() == name)
            return players[i]->get_price();
    return 0;
}

shared_ptr<Player> MyTeam::find_captain(string name)
{
    for (int i = 0; i < players.size(); i++)
        if (players[i]->get_name() == name)
            return players[i];
...
void MyTeam::print_players(shared_ptr<Player> captain)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        cout << name_of_roles[i] << ": " << name_of_players[i];
        if (captain != NULL)
            if (captain->get_name() == name_of_players[i])
                cout << " " << CAPTAIN_TAG;
        cout << endl;
    }
}

int MyTeam::find_total_cost()
{
    int sum = 0;
    for (int i = 0; i < players.size(); i++)
        sum = sum + players[i]->get_price();
    return sum;
}
...
float MyTeam::find_scores(vector<shared_ptr<Week>> weeks, int week_index, vector<shared_ptr<Player>> list_players, shared_ptr<Player> captain)
{
    float sum = 0;
    float new_score_to_add = 0;
    if (!is_complete())
        return sum;
    for (int i = 0; i < players.size(); i++)
        ...
        {
            new_score_to_add = weeks[week_index]->find_score_of_player(players[i]->get_name());
            if (captain != NULL)
                if (captain->get_name() == players[i]->get_name())
                {
                    float base_score = weeks[week_index]->find_base_score_of_player(players[i]->get_name()) * 2;
                    new_score_to_add = (int)((float)10.f / (1.f + exp(-base_score / 6.f)));
                }
            sum = sum + new_score_to_add;
        }
    return sum;
}
