...
shared_ptr<User> User::make_copy()
{
    ...
    copy_of_user->budget = budget;
    copy_of_user->captain = captain;
    return copy_of_user;
}
...
void User::sell_player(string name)
{
    ...
    status = my_team->sell_player(name,budget);
    if (status)
    {
        num_of_sell++;
        print(OK);
...
void User::buy_player(shared_ptr<Player> new_player)
{
    if (start && num_of_buy == 2)
        throw Permission_Denied();
    if (my_team->buy_player(new_player,budget))
    {
        budget -= new_player->get_price();
        num_of_buy++;
        print(OK);
    }
    else
...
{
    shared_ptr<Player> temp = my_team->find_captain(name);
    if (temp == NULL)
        throw Not_Found();
    captain = temp;
}
...
void User::print_squad()
{
    ...
    my_team->print_players(captain);
    cout << "Total Points: " << score << endl;
    cout << "Team Cost: " << my_team->find_total_cost() << endl;
}
...
void User::update_user(vector<shared_ptr<Week>> weeks, int week_index, vector<shared_ptr<Player>> list_players)
{
    ...
    if (my_team->is_complete())
        ...
        return;
    score = score + my_team->find_scores(weeks, week_index, list_players, captain);
}
