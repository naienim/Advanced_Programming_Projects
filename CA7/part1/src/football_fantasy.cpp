...
void FootballFantasy::find_players_of_team(shared_ptr<Team> new_team, vector<string> inputs)
{
    ...
    {
        players_name = make_apart(inputs[i], SEPARATOR2);
        for (int j = 0; j < players_name.size(); j++)
        {
            if (players_name[j].length() > 1)
            {
                new_team->add_player(correct_string(players_name[j]), roles[i - 1], 0);
                players.push_back(make_shared<Player>(correct_string(players_name[j]), roles[i - 1], 0));
            }
        }
    }
}
